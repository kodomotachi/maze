...
class maze:
    ...
    def _drawMaze(self, theme):
        ...
        self._win.title('PYTHON MAZE WORLDs')
        ...
        def create_new_maze():
            self.CreateMaze(loopPercent=100, theme=theme)  # tạo mê cung mới.

        # Hàm xử lý sự kiện cho nút "Solve"
        def solve_current_maze():
            path = DFS(self)  # tính toán đường đi.
            a = agent(self, footprints=True)  # tạo agent để giải mê cung.
            self.tracePath({a: path})  # giải mê cung.

        # Tạo nút "Create"
        create_button = Button(self._win, text="Create", command=create_new_maze)
        create_button.pack(side=LEFT)

        # Tạo nút "Solve"
        solve_button = Button(self._win, text="Solve", command=solve_current_maze)
