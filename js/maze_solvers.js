...
function depth_first()
{
    ...
    let stack = [start_pos];
    let parent_map = {}; // Lưu trữ cha của mỗi ô để truy vết đường đi
    grid[start_pos[0]][start_pos[1]] = 1;
    
    while (stack.length > 0 && !found)
    {
        let current = stack.pop(); // Lấy ô cuối cùng (Duyệt sâu)
        ...
        for (let i = 0; i < neighbors.length; i++)
        {
            ...
            if (get_node(nx, ny) === 0)
            {
                ...
                grid[nx][ny] = 1; // Đánh dấu đã đi qua
                parent_map[`${nx},${ny}`] = current; // Lưu cha để truy vết
            }
        }
    }
    ...
    if (found)
    {
        ...
        while (current_node[0] !== start_pos[0] || current_node[1] !== start_pos[1])
        {
            ...
            current_node = parent_map[`${current_node[0]},${current_node[1]}`];
        }
        path_list.reverse();
    }
